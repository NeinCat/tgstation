	...
	switch(action)
		if("ejectBeaker")
			if(beaker)
				...
				. = TRUE
		if("input")
			var/input_reagent = replacetext(r_lowertext(input("Enter the name of any reagent", "Input") as text|null), " ", "") //95% of the time, the reagent id is a lowercase/no spaces version of the name
