/proc/emoji_parse(text) //turns :ai: into an emoji in text.
	...
	while(1)
		...
		if(search)
			...
			if(search)
				emoji = r_lowertext(copytext(text, pos + length(text[pos]), search))
...
/proc/emoji_sanitize(text) //cuts any text that would not be parsed as an emoji
	...
	while(1)
		...
		if(search)
			...
			search = findtext(text, ":", pos + length(text[pos]))
			if(search)
				var/word = r_lowertext(copytext(text, pos + length(text[pos]), search))
				if(word in emojis)
					final += r_lowertext(copytext(text, pos, search + length(text[search])))
