...
/datum/round_event/wizard/deprevolt/start()
	...
	switch(tidecolor)
		...
		if("white")
			jobs_to_revolt = list("Prefect Medicus", "Medical Doctor", "Paramedic", "Chemist", "Geneticist", "Virologist")
			nation_name = pick("Mede", "Healtha", "Recova", "Chemi", "Geneti", "Viro", "Psych")
		if("yellow")
			jobs_to_revolt = list("Prefect Fectum", "Station Engineer", "Atmospheric Technician")
			nation_name = pick("Atomo", "Engino", "Power", "Teleco")
		if("purple")
			jobs_to_revolt = list("Prefect Scientia","Scientist", "Roboticist")
