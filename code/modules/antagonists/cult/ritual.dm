...
/obj/item/melee/cultblade/dagger/proc/scribe_rune_attempt(mob/living/user)
	...
	if(ispath(rune_to_scribe, /obj/effect/rune/narsie))
		...
		if(!(A in summon_objective.summon_spots))  // Check again to make sure they didn't move
			to_chat(user, "<span class='cultlarge'>The Geometer can only be summoned where the veil is weak - in [english_list(summon_objective.summon_spots)]!</span>")
			return
		priority_announce("Figments from an eldritch god are being summoned by [user] into [A.map_name] from an unknown dimension. Disrupt the ritual at all costs!","Imperial Senate Higher Dimensional Affairs", 'sound/ai/spanomalies.ogg')
	...
	to_chat(user, "<span class='cult'>The [r_lowertext(R.cultist_name)] rune [R.cultist_desc]</span>")
