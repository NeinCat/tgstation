...
/datum/antagonist/traitor/internal_affairs/proc/greet_iaa()
	...
	if(syndicate)
		...
		to_chat(owner.current, "<B><font size=5 color=red>Any damage you cause will be a further embarrassment to Empire, so you have no limits on collateral damage.</font></B>")
