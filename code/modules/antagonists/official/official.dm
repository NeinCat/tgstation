...
/datum/antagonist/official/greet()
	...
	if (ert_team)
		to_chat(owner, "Imperial Senate is sending you to [station_name()] with the task: [ert_team.mission.explanation_text]")
	else
		to_chat(owner, "Imperial Senate is sending you to [station_name()] with the task: [mission.explanation_text]")
...
/datum/antagonist/official/proc/forge_objectives()
	if (ert_team)
		...
		missionobj.explanation_text = "Conduct a routine performance review of [station_name()] and its Governor."
