...
/datum/controller/subsystem/ticker/Initialize(timeofday)
	...
	for(var/S in music)
		...
		if(L.len >= 2)
			var/ext = r_lowertext(L[L.len]) //pick the real extension, no 'honk.ogg.exe' nonsense here
	...
	if(!GLOB.accepted_edict)
		GLOB.accepted_edict = generate_random_edict()
...
/datum/controller/subsystem/ticker/proc/equip_characters()
	...
	for(var/i in GLOB.new_player_list)
		...
		if(istype(player) && player.mind && player.mind.assigned_role)
			if(player.mind.assigned_role == "Governor")
				captainless=0
	...
	if(captainless)
		for(var/i in GLOB.new_player_list)
			var/mob/dead/new_player/N = i
			if(N.new_character)
				to_chat(N, "<span class='notice'>Governorship not forced on anyone.</span>")
			CHECK_TICK
...
/datum/controller/subsystem/ticker/proc/send_news_report()
	var/news_message
	var/news_source = "Empire News Network"
	switch(news_report)
		...
		if(STATION_NUKED)
			news_message = "[station_name()] activated its self destruct device for unknown reasons. Attempts to clone the Governor so he can be arrested and executed are underway."
		if(CLOCK_SUMMON)
